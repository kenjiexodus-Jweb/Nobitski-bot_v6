...
const API_BASE = 'https://betadash-api-swordslush-production.up.railway.app/rip';
...
/** Fetch RIP image (buffer/url/text) */
async function fetchRipImage(userid) {
  ...
  const res = await axios.get(url, {
    ...
    maxRedirects: 5,
    validateStatus: null,
    headers: { 'User-Agent': 'ws3fca-rip-client', Accept: '*/*', Referer: API_BASE }
  });

  if (res.status < 200 || res.status >= 300) {
    let preview = '';
    try { preview = Buffer.from(res.data || []).toString('utf8').slice(0, 800); } catch (e) { preview = '(binary)'; }
    const err = new Error(`RIP API returned status ${res.status}: ${preview}`);
...
async function downloadUrlToFile(url, outPath) {
  const res = await axios.get(url, {
    ...
    timeout: 30000,
    maxRedirects: 5,
    headers: { 'User-Agent': 'ws3fca-rip-client', Referer: API_BASE, Accept: 'image/*,*/*' }
  });
...
module.exports = {
  config: {
    name: 'rip',
    version: '1.0',
    description: 'Send RIP edited image from API',
    hasPrefix: false,
    role: 0,
    guide: { en: 'rip <userid>  OR reply with: rip' }
  },
  ...
  async execute({ api, event, args }) {
    ...
    let { userid } = parsed;
    if (!userid) {
      const usage = 'Usage: rip <userid>\nExample: rip 100082770721408\nOr reply to a user message with: rip';
    ...
    try {
      loadingInfo = await new Promise((resolve) => {
        api.sendMessage(`⏳ Generating RIP image for ${userid}...`, threadID, (err, info) => {
          if (err) return resolve(null);
          return resolve(info || null);
        });
      });
    ...
    const tmpBase = `rip_${sanitizeFilename(userid)}_${Date.now()}`;
    let outFilePath = null;

    try {
      const fetched = await fetchRipImage(userid);
      ...
      if (fetched.kind === 'buffer') {
        ...
        await sendMessageAsync(api, `⚠️ RIP API returned no image: ${String(fetched.text || fetched.rawText || '').slice(0, 800)}`, threadID);
        throw new Error('No image from RIP API');
      }
      ...
      const headerText = '🕊️ 𝗥𝗜𝗣 𝗘𝗗𝗜𝗧𝗘𝗗 🕊️';
      ...
      if (!sent) {
        ...
        const ok = await new Promise((resolve) => {
          ...
          stream.on('error', (err) => console.error('[rip] stream error', err));
          api.sendMessage({ body: '', attachment: stream }, threadID, (err) => {
            if (err) { console.error('[rip] attachment send error', err); return resolve(false); }
            resolve(true);
          });
        });
        if (!ok) throw new Error('Failed to send RIP image attachment');
      }
      ...
      console.error('[rip] error:', err && (err.stack || err.message || err));
      try { await sendMessageAsync(api, `⚠️ Failed to generate/send RIP image: ${err.message || 'unknown'}`, threadID); } catch (e) {}
    } finally {
      // cleanup
