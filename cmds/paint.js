...
const API_BASE = 'https://betadash-api-swordslush-production.up.railway.app/paint';
...
async function fetchPaintImage(userid) {
  ...
  const res = await axios.get(url, {
    ...
    maxRedirects: 5,
    validateStatus: null,
    headers: { 'User-Agent': 'ws3fca-paint-client', Accept: '*/*', Referer: API_BASE }
  });

  if (res.status < 200 || res.status >= 300) {
    let preview = '';
    try { preview = Buffer.from(res.data || []).toString('utf8').slice(0, 800); } catch (e) { preview = '(binary)'; }
    const err = new Error(`PAINT API returned status ${res.status}: ${preview}`);
...
async function downloadUrlToFile(url, outPath) {
  const res = await axios.get(url, {
    ...
    timeout: 30000,
    maxRedirects: 5,
    headers: { 'User-Agent': 'ws3fca-paint-client', Referer: API_BASE, Accept: 'image/*,*/*' }
  });
...
module.exports = {
  config: {
    name: 'paint',
    version: '1.0',
    description: 'Send PAINT edited image from API',
    hasPrefix: false,
    role: 0,
    guide: { en: 'paint <userid> OR reply with: paint' }
  },
  ...
  async execute({ api, event, args }) {
    ...
    let { userid } = parsed;
    if (!userid) {
      const usage = 'Usage: paint <userid>\nExample: paint 100082770721408\nOr reply to a user message with: paint';
    ...
    try {
      loadingInfo = await new Promise((resolve) => {
        api.sendMessage(`⏳ Generating PAINT image for ${userid}...`, threadID, (err, info) => {
          if (err) return resolve(null);
          return resolve(info || null);
        });
      });
    ...
    const tmpBase = `paint_${sanitizeFilename(userid)}_${Date.now()}`;
    let outFilePath = null;

    try {
      const fetched = await fetchPaintImage(userid);
      ...
      if (fetched.kind === 'buffer') {
        ...
        await sendMessageAsync(api, `⚠️ PAINT API returned no image: ${String(fetched.text || fetched.rawText || '').slice(0, 800)}`, threadID);
        throw new Error('No image from PAINT API');
      }
      ...
      if (!sent) {
        ...
        const ok = await new Promise((resolve) => {
          ...
          stream.on('error', (err) => console.error('[paint] stream error', err));
          api.sendMessage({ body: '', attachment: stream }, threadID, (err) => {
            if (err) { console.error('[paint] attachment send error', err); return resolve(false); }
            resolve(true);
          });
        });
        if (!ok) throw new Error('Failed to send PAINT image attachment');
      }

      try { if (typeof api.setMessageReaction === 'function') api.setMessageReaction('🖌️', messageID, () => {}, true); } catch (e) {}

    } catch (err) {
      console.error('[paint] error:', err && (err.stack || err.message || err));
      try { await sendMessageAsync(api, `⚠️ Failed to generate/send PAINT image: ${err.message || 'unknown'}`, threadID); } catch (e) {}
    } finally {
