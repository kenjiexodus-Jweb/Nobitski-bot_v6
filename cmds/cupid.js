...
const API_BASE = 'https://betadash-api-swordslush-production.up.railway.app/cupid';
...
async function fetchCupidImage(userid) {
  ...
  const res = await axios.get(url, {
    ...
    maxRedirects: 5,
    validateStatus: null,
    headers: { 'User-Agent': 'ws3fca-cupid-client', Accept: '*/*', Referer: API_BASE }
  });

  if (res.status < 200 || res.status >= 300) {
    let preview = '';
    try { preview = Buffer.from(res.data || []).toString('utf8').slice(0, 800); } catch (e) { preview = '(binary)'; }
    const err = new Error(`Cupid API returned status ${res.status}: ${preview}`);
...
async function downloadUrlToFile(url, outPath) {
  const res = await axios.get(url, {
    ...
    timeout: 30000,
    maxRedirects: 5,
    headers: { 'User-Agent': 'ws3fca-cupid-client', Referer: API_BASE, Accept: 'image/*,*/*' }
  });
...
module.exports = {
  config: {
    name: 'cupid',
    version: '1.0',
    description: 'Send cupid edited image from API',
    hasPrefix: false,
    role: 0,
    guide: { en: 'cupid <userid>  OR reply with: cupid' }
  },
  ...
  async execute({ api, event, args }) {
    ...
    let { userid } = parsed;
    if (!userid) {
      const usage = 'Usage: cupid <userid>\nExample: cupid 100082770721408\nOr reply to a user message with: cupid';
    ...
    try {
      loadingInfo = await new Promise((resolve) => {
        api.sendMessage(`⏳ Generating cupid image for ${userid}...`, threadID, (err, info) => {
          if (err) return resolve(null);
          resolve(info || null);
        });
      });
    ...
    const tmpBase = `cupid_${sanitizeFilename(userid)}_${Date.now()}`;
    let outFilePath = null;

    try {
      const fetched = await fetchCupidImage(userid);
      ...
      if (fetched.kind === 'buffer') {
        ...
        await sendMessageAsync(api, `⚠️ Cupid API returned no image: ${String(fetched.text || fetched.rawText || '').slice(0, 800)}`, threadID);
        throw new Error('No image from Cupid API');
      }
      ...
      // header text (Unicode-ish bold)
      const headerText = '💘 𝗖𝗨𝗣𝗜𝗗 𝗘𝗗𝗜𝗧𝗘𝗗 💘';

      // Try sending as single message with attachment
      let sent = false;
      try {
      ...
      // fallback: send text then image
      if (!sent) {
        try { await sendMessageAsync(api, headerText, threadID); } catch (e) {}
        const ok = await new Promise((resolve) => {
          const stream = fs.createReadStream(outFilePath);
          stream.on('error', (err) => console.error('[cupid] stream error', err));
          api.sendMessage({ body: '', attachment: stream }, threadID, (err) => {
            if (err) { console.error('[cupid] attachment send error', err); return resolve(false); }
            resolve(true);
          });
        });
        if (!ok) throw new Error('Failed to send cupid image attachment');
      }
      ...
      console.error('[cupid] error:', err && (err.stack || err.message || err));
      try { await sendMessageAsync(api, `⚠️ Failed to generate/send cupid image: ${err.message || 'unknown'}`, threadID); } catch (e) {}
    } finally {
      // cleanup file
