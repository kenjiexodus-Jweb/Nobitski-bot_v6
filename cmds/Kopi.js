...
const configPath = path.join(dataDir, "nescafe-config.json");
...
module.exports = {
  ...
  execute: async ({ api, event }) => {
    ...
    if (!fs.existsSync(configPath)) fs.writeJsonSync(configPath, { on: true, name: "Nescafe", prefix: "!" });
    ...
    const botName = botConfig.name || "Nescafe";

    // 🧠 Teach new phrase
    if (text.startsWith("nescafe teach ")) {
      const parts = text.replace("nescafe teach ", "").split("/");
      if (parts.length !== 2) {
        return api.sendMessage(
          `❌ Invalid format. Use:\nnescafe teach <question>/<reply>`,
          event.threadID,
          event.messageID
        );
      }
