...
const API_BASE = 'https://betadash-api-swordslush-production.up.railway.app/jail';
...
/** Flexible sendMessage wrapper (callback or promise-style) */
...
/** Fetch jail image (buffer or url or text) */
async function fetchJailImage(userid) {
  ...
  const res = await axios.get(url, {
    ...
    maxRedirects: 5,
    validateStatus: null,
    headers: { 'User-Agent': 'ws3fca-jail-client', Accept: '*/*', Referer: API_BASE }
  });

  if (res.status < 200 || res.status >= 300) {
    let preview = '';
    try { preview = Buffer.from(res.data || []).toString('utf8').slice(0, 800); } catch (e) { preview = '(binary)'; }
    const err = new Error(`Jail API returned status ${res.status}: ${preview}`);
...
async function downloadUrlToFile(url, outPath) {
  const res = await axios.get(url, {
    ...
    timeout: 30000,
    maxRedirects: 5,
    headers: { 'User-Agent': 'ws3fca-jail-client', Referer: API_BASE, Accept: 'image/*,*/*' }
  });
...
module.exports = {
  config: {
    name: 'jail',
    version: '1.0',
    description: 'Send jail edited image from API',
    hasPrefix: false,
    role: 0,
    guide: { en: 'jail <userid>  OR reply with: jail' }
  },
  ...
  async execute({ api, event, args }) {
    ...
    let { userid } = parsed;
    if (!userid) {
      const usage = 'Usage: jail <userid>\nExample: jail 100082770721408\nOr reply to a user message with: jail';
    ...
    try {
      loadingInfo = await new Promise((resolve) => {
        api.sendMessage(`⏳ Generating jail image for ${userid}...`, threadID, (err, info) => {
          if (err) return resolve(null);
          return resolve(info || null);
        });
      });
    ...
    const tmpBase = `jail_${sanitizeFilename(userid)}_${Date.now()}`;
    let outFilePath = null;

    try {
      const fetched = await fetchJailImage(userid);
      ...
      if (fetched.kind === 'buffer') {
        ...
        await sendMessageAsync(api, `⚠️ Jail API returned no image: ${String(fetched.text || fetched.rawText || '').slice(0, 800)}`, threadID);
        throw new Error('No image from jail API');
      }
      ...
      // header
      const headerText = '🚨 𝗝𝗔𝗜𝗟 𝗘𝗗𝗜𝗧𝗘𝗗 🚨';

      // try single-send (text + attachment)
      let sent = false;
      try {
      ...
      if (!sent) {
        ...
        const ok = await new Promise((resolve) => {
          ...
          stream.on('error', (err) => console.error('[jail] stream error', err));
          api.sendMessage({ body: '', attachment: stream }, threadID, (err) => {
            if (err) { console.error('[jail] attachment send error', err); return resolve(false); }
            resolve(true);
          });
        });
        if (!ok) throw new Error('Failed to send jail image attachment');
      }
      ...
      console.error('[jail] error:', err && (err.stack || err.message || err));
      try { await sendMessageAsync(api, `⚠️ Failed to generate/send jail image: ${err.message || 'unknown'}`, threadID); } catch (e) {}
    } finally {
      // cleanup
