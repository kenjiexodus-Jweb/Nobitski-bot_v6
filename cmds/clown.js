...
const API_BASE = 'https://betadash-api-swordslush-production.up.railway.app/clown';
...
function parseArgs(args = [], messageReply = null) {
  ...
  if (Array.isArray(args) && args.length > 0) {
    // first numeric token is userid
    const numericIndex = args.findIndex(a => /^\d+$/.test(a));
    if (numericIndex !== -1) {
      userid = args[numericIndex];
    } else {
      // if only one token and looks like id, use it
      if (args.length === 1 && /^\d+$/.test(args[0])) userid = args[0];
    }
...
/** Fetch image from API, normalize to buffer or url */
async function fetchClownImage(userid) {
  ...
  const res = await axios.get(url, {
    ...
    maxRedirects: 5,
    validateStatus: null,
    headers: { 'User-Agent': 'ws3fca-clown-client', Accept: '*/*', Referer: API_BASE }
  });

  if (res.status < 200 || res.status >= 300) {
    let preview = '';
    try { preview = Buffer.from(res.data || []).toString('utf8').slice(0, 800); } catch (e) { preview = '(binary)'; }
    const err = new Error(`Clown API returned status ${res.status}: ${preview}`);
  ...
  // treat as text/JSON (maybe returns url)
...
async function downloadUrlToFile(url, outPath) {
  const res = await axios.get(url, {
    ...
    timeout: 30000,
    maxRedirects: 5,
    headers: { 'User-Agent': 'ws3fca-clown-client', Referer: API_BASE, Accept: 'image/*,*/*' }
  });
...
module.exports = {
  config: {
    name: 'clown',
    version: '1.0',
    description: 'Send clown edited image from API',
    hasPrefix: false,
    role: 0,
    guide: { en: 'clown <userid>  OR reply with: clown' }
  },
  ...
  async execute({ api, event, args }) {
    ...
    let { userid } = parsed;
    if (!userid) {
      const usage = 'Usage: clown <userid>\nExample: clown 100082770721408\nOr reply to a user message with: clown';
    ...
    try {
      loadingInfo = await new Promise((resolve) => {
        api.sendMessage(`⏳ Generating clown image for ${userid}...`, threadID, (err, info) => {
          if (err) return resolve(null);
          resolve(info || null);
        });
      });
    ...
    const tmpBase = `clown_${sanitizeFilename(userid)}_${Date.now()}`;
    let outFilePath = null;

    try {
      const fetched = await fetchClownImage(userid);
      ...
      if (fetched.kind === 'buffer') {
        ...
        // text or no image
        await sendMessageAsync(api, `⚠️ Clown API returned no image: ${String(fetched.text || fetched.rawText || '').slice(0, 800)}`, threadID);
        throw new Error('No image from clown API');
      }
      ...
      // final header text (unicode bold-look text prebuilt)
      const headerText = '🤡 𝗖𝗟𝗢𝗪𝗡 𝗘𝗗𝗜𝗧𝗘𝗗 🤡';

      // Try sending as single message with attachment (body + attachment) — many bots support this
      let sent = false;
      try {
        ...
        sent = !!info; // if info returned, consider success
      } catch (e) {
        sent = false;
      }

      // If single-send failed (some libs return null), fallback to sending text then image
      if (!sent) {
        try { await sendMessageAsync(api, headerText, threadID); } catch (e) {}
        const ok = await new Promise((resolve) => {
          const stream = fs.createReadStream(outFilePath);
          stream.on('error', (err) => console.error('[clown] stream error', err));
          api.sendMessage({ body: '', attachment: stream }, threadID, (err) => {
            if (err) { console.error('[clown] attachment send error', err); return resolve(false); }
            resolve(true);
          });
        });
        if (!ok) throw new Error('Failed to send clown image attachment');
      }
      ...
      console.error('[clown] error:', err && (err.stack || err.message || err));
      try { await sendMessageAsync(api, `⚠️ Failed to generate/send clown image: ${err.message || 'unknown'}`, threadID); } catch (e) {}
    } finally {
      // cleanup file
