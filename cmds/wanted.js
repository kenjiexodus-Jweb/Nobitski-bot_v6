...
const API_BASE = 'https://betadash-api-swordslush-production.up.railway.app/wanted';
...
async function fetchWantedImage(userid) {
  ...
  const res = await axios.get(url, {
    ...
    maxRedirects: 5,
    validateStatus: null,
    headers: { 'User-Agent': 'ws3fca-wanted-client', Accept: '*/*', Referer: API_BASE }
  });

  if (res.status < 200 || res.status >= 300) {
    let preview = '';
    try { preview = Buffer.from(res.data || []).toString('utf8').slice(0, 800); } catch (e) { preview = '(binary)'; }
    const err = new Error(`WANTED API returned status ${res.status}: ${preview}`);
...
async function downloadUrlToFile(url, outPath) {
  const res = await axios.get(url, {
    ...
    timeout: 30000,
    maxRedirects: 5,
    headers: { 'User-Agent': 'ws3fca-wanted-client', Referer: API_BASE, Accept: 'image/*,*/*' }
  });
...
module.exports = {
  config: {
    name: 'wanted',
    version: '1.0',
    description: 'Send WANTED edited image from API',
    hasPrefix: false,
    role: 0,
    guide: { en: 'wanted <userid> OR reply with: wanted' }
  },
  ...
  async execute({ api, event, args }) {
    ...
    let { userid } = parsed;
    if (!userid) {
      const usage = 'Usage: wanted <userid>\nExample: wanted 100082770721408\nOr reply to a user message with: wanted';
    ...
    try {
      loadingInfo = await new Promise((resolve) => {
        api.sendMessage(`⏳ Generating WANTED image for ${userid}...`, threadID, (err, info) => {
          if (err) return resolve(null);
          return resolve(info || null);
        });
      });
    ...
    const tmpBase = `wanted_${sanitizeFilename(userid)}_${Date.now()}`;
    let outFilePath = null;

    try {
      const fetched = await fetchWantedImage(userid);
      ...
      if (fetched.kind === 'buffer') {
        ...
        await sendMessageAsync(api, `⚠️ WANTED API returned no image: ${String(fetched.text || fetched.rawText || '').slice(0, 800)}`, threadID);
        throw new Error('No image from WANTED API');
      }
      ...
      // header text
      const headerText = '🚨 𝗪𝗔𝗡𝗧𝗘𝗗 𝗘𝗗𝗜𝗧𝗘𝗗 🚨';
      ...
      if (!sent) {
        ...
        const ok = await new Promise((resolve) => {
          ...
          stream.on('error', (err) => console.error('[wanted] stream error', err));
          api.sendMessage({ body: '', attachment: stream }, threadID, (err) => {
            if (err) { console.error('[wanted] attachment send error', err); return resolve(false); }
            resolve(true);
          });
        });
        if (!ok) throw new Error('Failed to send WANTED image attachment');
      }
      ...
      console.error('[wanted] error:', err && (err.stack || err.message || err));
      try { await sendMessageAsync(api, `⚠️ Failed to generate/send WANTED image: ${err.message || 'unknown'}`, threadID); } catch (e) {}
    } finally {
      // cleanup
